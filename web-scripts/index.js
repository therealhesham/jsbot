(function() {
    ...
    try {
      ...
      if (visaCenterSelect) {
        ...
        setTimeout(() => {
            ...
            if (location.href.includes("sau/en/fra/application-detail") && !HTMLElm.innerText.includes('No appointment slots are currently available') && !continueBtn) {
  ...
  setInterval(() => {
    ...
    if (locationHref.includes("sau/en/fra/login")) {
      autoLogin()
    } else if (locationHref.includes("sau/en/fra/dashboard")) {
      startNewBooking()
    } else if (locationHref.includes("sau/en/fra/application-detail")) {
      if (canReset) {
      ...
      if (shoudlRefresh) {
        location.href = "https://visa.vfsglobal.com/sau/en/fra/login"
        return
      }
